...
(function () {  'use strict';  const STORAGE_KEY = 'work_dashboard_tasks_v1';  const WEEKLY_KEY = 'work_weekly_history_v1';  const MONTHLY_KEY = 'work_monthly_history_v1';  const PROJECT_ORDER_KEY = 'work_project_order_v1';  const PROJECT_COLLAPSE_KEY = 'work_project_collapse_v1';  const PROJECT_NOTES_KEY = 'work_project_notes_v1';  const STATUS_LIST = ['미진행', '진행중', '대기', '완료'];  const DEFAULT_TASK_STATUS_FILTERS = STATUS_LIST.slice(0, 2);  const CHANNEL_LIST = ['웍스', '메일', '전화', '회의', '구두', '기타'];  const STATUS_COLORS = {    '완료': '#34c759',    '진행중': '#007aff',    '대기': '#af52de',    '미진행': '#8e8e93'  };  const CHANNEL_COLORS = ['#007aff', '#34c759', '#ff9500', '#af52de', '#5ac8fa', '#8e8e93'];  let tasks = [];  let weeklyHistory = [];  let monthlyHistory = [];  let projectOrder = [];  let projectNotes = {};  let selectedPriority = '중';  const ACTIVE_TAB_KEY = 'work_board_active_tab';  const TASK_STATUS_FILTER_KEY = 'work_board_task_status_filters';  let currentTab = 'home';  let taskFilterStatuses = new Set(DEFAULT_TASK_STATUS_FILTERS);  let currentPage = 1;  let rowsPerPage = 10;  let projectFilter = '전체';  let weeklyOffset = 0;  let monthlyOffset = 0;  let dueAmpm = '오전';  let statusChartWindow = 'day';  let channelChartWindow = 'week';  let trendChartWindow = '7d';  const expandedWeeklyReports = new Set();  const expandedMonthlyReports = new Set();  const collapsedProjectGroups = new Set();  const $ = (id) => document.getElementById(id);  const escapeHtml = (value) => String(value ?? '')    .replace(/&/g, '&amp;')    .replace(/</g, '&lt;')    .replace(/>/g, '&gt;')    .replace(/"/g, '&quot;')    .replace(/'/g, '&#39;');  function safeArrayParse(raw) {    if (!raw) return [];    try {      const parsed = JSON.parse(raw);      return Array.isArray(parsed) ? parsed : [];    } catch (error) {      console.error('JSON parse error:', error);      return [];    }  }  function readObjectStorage(key) {    const raw = localStorage.getItem(key);    if (!raw) return {};    try {      const parsed = JSON.parse(raw);      return parsed && typeof parsed === 'object' && !Array.isArray(parsed) ? parsed : {};    } catch (error) {      console.error('JSON parse error:', error);      return {};    }  }  function loadTaskStatusFilters() {    const saved = safeArrayParse(localStorage.getItem(TASK_STATUS_FILTER_KEY)).filter((status) => status === '전체' || STATUS_LIST.includes(status));    taskFilterStatuses = new Set(saved.length ? saved : DEFAULT_TASK_STATUS_FILTERS);    if (taskFilterStatuses.has('전체') || taskFilterStatuses.size === 0) taskFilterStatuses = new Set(['전체']);  }  function saveTaskStatusFilters() {    localStorage.setItem(TASK_STATUS_FILTER_KEY, JSON.stringify(Array.from(taskFilterStatuses)));  }  function getActiveTaskStatuses() {    return taskFilterStatuses.has('전체') ? STATUS_LIST : STATUS_LIST.filter((status) => taskFilterStatuses.has(status));  }  function updateStatusFilterButtons() {    document.querySelectorAll('[data-status-filter]').forEach((btn) => {      btn.classList.toggle('active', taskFilterStatuses.has(btn.dataset.statusFilter));    });  }  function setTaskStatusFilter(status) {    if (status === '전체') {      taskFilterStatuses = new Set(['전체']);    } else {      taskFilterStatuses.delete('전체');      if (taskFilterStatuses.has(status)) taskFilterStatuses.delete(status);      else taskFilterStatuses.add(status);      if (taskFilterStatuses.size === 0) taskFilterStatuses.add('전체');    }    saveTaskStatusFilters();    updateStatusFilterButtons();    currentPage = 1;    renderTaskTable();  }  function pad2(n) { return String(n).padStart(2, '0'); }  function startOfDay(date) {    const d = new Date(date);    d.setHours(0, 0, 0, 0);    return d;  }  function endOfDay(date) {    const d = new Date(date);    d.setHours(23, 59, 59, 999);    return d;  }  function addDays(date, days) {    const d = new Date(date);    d.setDate(d.getDate() + days);    return d;  }  function getWeekRange(offset = 0) {    const now = new Date();    const base = startOfDay(now);    const day = base.getDay();    const mondayShift = day === 0 ? -6 : 1 - day;    const start = addDays(base, mondayShift + offset * 7);    const end = endOfDay(addDays(start, 6));    return { start, end };  }  function getMonthRange(offset = 0) {    const now = new Date();    const start = new Date(now.getFullYear(), now.getMonth() + offset, 1, 0, 0, 0, 0);    const end = new Date(now.getFullYear(), now.getMonth() + offset + 1, 0, 23, 59, 59, 999);    return { start, end };  }  function parseKoreanTime(timeStr) {    if (!timeStr || typeof timeStr !== 'string') return { hour: 12, minute: 0 };    const trimmed = timeStr.trim();    const m = trimmed.match(/^(오전|오후)\s*(\d{1,2}):(\d{2})$/);    if (m) {      let hour = Number(m[2]);      const minute = Number(m[3]);      if (m[1] === '오전') {        if (hour === 12) hour = 0;      } else {        if (hour !== 12) hour += 12;      }      return { hour, minute };    }    const m2 = trimmed.match(/^(\d{1,2}):(\d{2})$/);    if (m2) {      return { hour: Number(m2[1]), minute: Number(m2[2]) };    }    return { hour: 12, minute: 0 };  }  function toKoreanTime(date) {    const h = date.getHours();    const ap = h >= 12 ? '오후' : '오전';    const hh = h % 12 || 12;    return `${ap} ${pad2(hh)}:${pad2(date.getMinutes())}`;  }  function parseDueDateTime(dueDate, dueTime) {    if (!dueDate) return null;    const parts = String(dueDate).split('-').map(Number);    if (parts.length !== 3 || parts.some(Number.isNaN)) return null;    const t = parseKoreanTime(dueTime);    return new Date(parts[0], parts[1] - 1, parts[2], t.hour, t.minute, 0, 0);  }  function formatDueDisplay(task) {    const d = parseDueDateTime(task.dueDate, task.dueTime);    if (!d) return '-';    return `${d.getFullYear()}.${pad2(d.getMonth() + 1)}.${pad2(d.getDate())} ${toKoreanTime(d)}`;  }  function normalizeTask(raw, index) {    const created = raw && raw.createdAt ? new Date(raw.createdAt) : new Date();    const createdAt = Number.isNaN(created.getTime()) ? new Date().toISOString() : created.toISOString();    const priority = ['상', '중', '하'].includes(raw?.priority) ? raw.priority : '중';    const normalizedStatus = raw?.status === '미착수' ? '미진행' : raw?.status;
    const status = STATUS_LIST.includes(normalizedStatus) ? normalizedStatus : '미진행';    const channel = CHANNEL_LIST.includes(raw?.channel) ? raw.channel : '기타';    const toPriorityLevel = (value, fallback) => { const n = Number(value); return [1,2,3].includes(n) ? n : fallback; };    const fallbackImpact = priority === '상' ? 3 : priority === '중' ? 2 : 1;    const fallbackUrgency = priority === '상' ? 3 : priority === '중' ? 2 : 1;    const fallbackLinkage = priority === '상' ? 2 : 1;    return {      id: raw?.id ? String(raw.id) : `task_${Date.now()}_${index}`,      name: String(raw?.name ?? '').trim(),      channel,      dueDate: typeof raw?.dueDate === 'string' ? raw.dueDate : '',      dueTime: typeof raw?.dueTime === 'string' ? raw.dueTime : '',      priority,      status,      project: typeof raw?.project === 'string' ? raw.project : '',      description: typeof raw?.description === 'string' ? raw.description : '',      outcome: typeof raw?.outcome === 'string' ? raw.outcome : '',      learning: typeof raw?.learning === 'string' ? raw.learning : '',      impact: toPriorityLevel(raw?.impact, fallbackImpact),      urgency: toPriorityLevel(raw?.urgency, fallbackUrgency),      linkage: toPriorityLevel(raw?.linkage, fallbackLinkage),      createdAt    };  }  function normalizeReport(raw, index, type) {    const savedAt = raw?.savedAt ? new Date(raw.savedAt) : new Date();    const safeSavedAt = Number.isNaN(savedAt.getTime()) ? new Date().toISOString() : savedAt.toISOString();    return {      id: raw?.id ? String(raw.id) : `${type}_${Date.now()}_${index}`,      periodKey: String(raw?.periodKey ?? ''),      title: String(raw?.title ?? ''),      content: String(raw?.content ?? ''),      savedAt: safeSavedAt,      savedAtText: raw?.savedAtText ? String(raw.savedAtText) : new Date(safeSavedAt).toLocaleString('ko-KR')    };  }  function loadStorage() {    collapsedProjectGroups.clear();    tasks = safeArrayParse(localStorage.getItem(STORAGE_KEY))      .map(normalizeTask)      .filter((task) => task.name);    weeklyHistory = safeArrayParse(localStorage.getItem(WEEKLY_KEY)).map((item, index) => normalizeReport(item, index, 'weekly'));    const rawProjectNotes = readObjectStorage(PROJECT_NOTES_KEY);    projectNotes = rawProjectNotes && typeof rawProjectNotes === 'object' && !Array.isArray(rawProjectNotes) ? rawProjectNotes : {};    monthlyHistory = safeArrayParse(localStorage.getItem(MONTHLY_KEY)).map((item, index) => normalizeReport(item, index, 'monthly'));    projectOrder = safeArrayParse(localStorage.getItem(PROJECT_ORDER_KEY)).map((item) => String(item || '').trim()).filter(Boolean);    safeArrayParse(localStorage.getItem(PROJECT_COLLAPSE_KEY)).map((item) => String(item || '').trim()).filter(Boolean).forEach((name) => collapsedProjectGroups.add(name));    syncProjectOrder();  }  function saveTasks() {    localStorage.setItem(STORAGE_KEY, JSON.stringify(tasks));    syncProjectOrder();  }  function saveWeeklyHistory() {    localStorage.setItem(WEEKLY_KEY, JSON.stringify(weeklyHistory));  }  function saveMonthlyHistory() {    localStorage.setItem(MONTHLY_KEY, JSON.stringify(monthlyHistory));  }  function saveProjectOrder() {    localStorage.setItem(PROJECT_ORDER_KEY, JSON.stringify(projectOrder));  }  function saveProjectNotes() {    localStorage.setItem(PROJECT_NOTES_KEY, JSON.stringify(projectNotes));  }  function saveCollapsedProjectGroups() {    localStorage.setItem(PROJECT_COLLAPSE_KEY, JSON.stringify(Array.from(collapsedProjectGroups)));  }  function getDistinctProjectNames() {    return Array.from(new Set(tasks.map((task) => String(task.project || '').trim()).filter(Boolean)));  }  function syncProjectOrder() {    const names = getDistinctProjectNames();    const before = JSON.stringify(projectOrder);    projectOrder = projectOrder.filter((name) => names.includes(name));    names.forEach((name) => { if (!projectOrder.includes(name)) projectOrder.push(name); });    if (JSON.stringify(projectOrder) !== before) saveProjectOrder();  }  function getOrderedProjectGroups(groupMap) {    const names = Object.keys(groupMap);    const orderedNamed = projectOrder.filter((name) => names.includes(name));    names.filter((name) => name !== '__unassigned__' && !orderedNamed.includes(name)).sort((a,b)=>a.localeCompare(b,'ko')).forEach((name) => orderedNamed.push(name));    if (names.includes('__unassigned__')) orderedNamed.push('__unassigned__');    return orderedNamed.map((name) => [name, groupMap[name]]);  }  function moveProjectOrder(projectName, direction) {    if (!projectName || projectName === '__unassigned__') return;    syncProjectOrder();    const index = projectOrder.indexOf(projectName);    if (index < 0) return;    const target = direction === 'up' ? index - 1 : index + 1;    if (target < 0 || target >= projectOrder.length) return;    const swap = projectOrder[target];    projectOrder[target] = projectName;    projectOrder[index] = swap;    saveProjectOrder();    renderTaskTable();  }  function renameProject(currentName) {    if (!currentName || currentName === '__unassigned__') return;    const nextName = prompt('변경할 프로젝트 명칭을 입력하세요.', currentName);    if (nextName == null) return;    const trimmed = String(nextName).trim();    if (!trimmed || trimmed === currentName) return;    tasks.forEach((task) => { if (String(task.project || '').trim() === currentName) task.project = trimmed; });    projectOrder = projectOrder.map((name) => name === currentName ? trimmed : name).filter((name, index, arr) => arr.indexOf(name) === index);    if (projectNotes[currentName] && !projectNotes[trimmed]) { projectNotes[trimmed] = { ...projectNotes[currentName], updatedAt: new Date().toISOString() }; delete projectNotes[currentName]; saveProjectNotes(); }    if (collapsedProjectGroups.has(currentName)) { collapsedProjectGroups.delete(currentName); collapsedProjectGroups.add(trimmed); }    saveTasks();    saveCollapsedProjectGroups();    refreshProjectFilterOptions();    renderProjectSelectOptions();    renderAll();  }  function toggleProjectGroup(projectName) {    if (!projectName) return;    if (collapsedProjectGroups.has(projectName)) collapsedProjectGroups.delete(projectName); else collapsedProjectGroups.add(projectName);    saveCollapsedProjectGroups();    renderTaskTable();  }  function renderProjectSelectOptions(selectedProject = '', keepInput = '') {
    syncProjectOrder();
    const orderedProjects = projectOrder.slice();
    const normalizedSelected = String(selectedProject || '').trim();
    const currentQuickInput = typeof keepInput === 'string' ? keepInput : ($('taskProject')?.value || '');
    const quickSelect = $('taskProjectSelect');
    if (quickSelect) {
      quickSelect.innerHTML = `<option value="__new__">새로 입력</option>${orderedProjects.map((name) => `<option value="${escapeHtml(name)}">${escapeHtml(name)}</option>`).join('')}`;
      quickSelect.value = normalizedSelected && orderedProjects.includes(normalizedSelected) ? normalizedSelected : '__new__';
    }
    if ($('taskProject')) $('taskProject').value = normalizedSelected || currentQuickInput || '';
    const select = $('detailTaskProjectSelect');
    if (select) {
      const currentInput = typeof keepInput === 'string' ? keepInput : ($('detailTaskProject')?.value || '');
      select.innerHTML = `<option value="__new__">새로 입력</option>${orderedProjects.map((name) => `<option value="${escapeHtml(name)}">${escapeHtml(name)}</option>`).join('')}`;
      select.value = normalizedSelected && orderedProjects.includes(normalizedSelected) ? normalizedSelected : '__new__';
      if ($('detailTaskProject')) $('detailTaskProject').value = normalizedSelected || currentInput || '';
    }
  }  function isCreatedWithin(task, start, end) {    const d = new Date(task.createdAt);    return !Number.isNaN(d.getTime()) && d >= start && d <= end;  }  function isDueWithin(task, start, end) {    const d = parseDueDateTime(task.dueDate, task.dueTime);    return !!d && d >= start && d <= end;  }  function isRelatedWithin(task, start, end) {    return isCreatedWithin(task, start, end) || isDueWithin(task, start, end);  }  function getDueDate(task) {    return parseDueDateTime(task.dueDate, task.dueTime);  }  function dueDayDistance(task) {    const d = getDueDate(task);    if (!d) return null;    return Math.round((startOfDay(d) - startOfDay(new Date())) / 86400000);  }  function isDueToday(task) {    if (task.status === '완료') return false;    return dueDayDistance(task) === 0;  }  function isPastDueDay(task) {    if (task.status === '완료') return false;    const distance = dueDayDistance(task);    return distance != null && distance < 0;  }  function isOverdue(task) {    if (task.status === '완료') return false;    const d = getDueDate(task);    return !!d && d.getTime() < Date.now();  }  function isDueSoon(task) {    if (task.status === '완료') return false;    const distance = dueDayDistance(task);    return distance != null && distance > 0 && distance <= 3;  }  function urgencyGroup(task) {    if (task.status === '완료') return 5;    if (isPastDueDay(task)) return 0;    if (isDueToday(task)) return 1;    if (isDueSoon(task)) return 2;    const due = getDueDate(task);    return due ? 3 : 4;  }  function getPriorityInputs(task) {    const impact = [1,2,3].includes(Number(task?.impact)) ? Number(task.impact) : (task?.priority === '상' ? 3 : task?.priority === '하' ? 1 : 2);    const urgency = [1,2,3].includes(Number(task?.urgency)) ? Number(task.urgency) : (task?.priority === '상' ? 3 : task?.priority === '하' ? 1 : 2);    const linkage = [1,2,3].includes(Number(task?.linkage)) ? Number(task.linkage) : (task?.priority === '상' ? 2 : 1);    return { impact, urgency, linkage };  }  function getBasePriorityScore(task) {    const { impact, urgency, linkage } = getPriorityInputs(task);    return impact + urgency + linkage;  }  function getDueDateWeight(task) {    if (task?.status === '완료') return 0;    const distance = dueDayDistance(task);    if (distance == null) return 0;    if (distance < 0) return 7;    if (distance === 0) return 8;    if (distance === 1) return 5;    if (distance <= 3) return 3;    return 1;  }  function getPriorityScore(task) {    return getBasePriorityScore(task) + getDueDateWeight(task);  }  function getPriorityBadge(score) {    if (score >= 8) return { cls: 'score-critical', label: 'Critical' };    if (score >= 6) return { cls: 'score-urgent', label: 'High' };    if (score >= 4) return { cls: 'score-watch', label: 'Watch' };    return { cls: 'score-low', label: 'Low' };  }  function getDueBadgeDisplay(task) {    const distance = dueDayDistance(task);    if (distance == null) return null;    if (distance < 0) return { cls: 'dday-overdue', label: 'D+' + Math.abs(distance) };    if (distance === 0) return { cls: 'dday-today', label: 'D-DAY' };    if (distance <= 3) return { cls: 'dday-soon', label: 'D-' + distance };    return { cls: 'dday-normal', label: 'D-' + distance };  }  function getPriorityDisplay(task) {    const score = getPriorityScore(task);    const badge = getPriorityBadge(score);    const label = badge.label;    const text = badge.label;    return { score, label, cls: badge.cls, level: badge.label, text };  }  function calcScore(task) {    if (task.status === '완료') return -1;    let score = getPriorityScore(task);    if (isPastDueDay(task)) score += 4;    else if (isDueToday(task)) score += 5;    else if (isDueSoon(task)) score += 2;    if (task.status === '진행중') score += 1;    return score;  }  function sortTasks(list) {    return [...list].sort((a, b) => {      const scoreDiff = calcScore(b) - calcScore(a);      if (scoreDiff !== 0) return scoreDiff;      const priorityDiff = getPriorityScore(b) - getPriorityScore(a);      if (priorityDiff !== 0) return priorityDiff;      const da = parseDueDateTime(a.dueDate, a.dueTime)?.getTime() ?? Number.MAX_SAFE_INTEGER;      const db = parseDueDateTime(b.dueDate, b.dueTime)?.getTime() ?? Number.MAX_SAFE_INTEGER;      if (da !== db) return da - db;      return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime();    });  }  function fillTimeSelectorsFromDue(hour24, minute) {
  ...
  function updateTaskFormPriorityPreview() {    const dueDate = $('taskDueDate')?.value || '';    const tempTask = {      impact: Number($('taskImpact')?.value || 2),      urgency: Number($('taskUrgency')?.value || 2),      linkage: Number($('taskLinkage')?.value || 1),      dueDate,      dueTime: dueDate ? getFormDueTimeValue() : ''    };    const priority = getPriorityDisplay(tempTask);    if ($('taskPriorityPreview')) $('taskPriorityPreview').value = priority.text;  }  function setTaskPriorityInputs(task) {    const inputs = getPriorityInputs(task);    if ($('taskImpact')) $('taskImpact').value = String(inputs.impact);    if ($('taskUrgency')) $('taskUrgency').value = String(inputs.urgency);    if ($('taskLinkage')) $('taskLinkage').value = String(inputs.linkage);    updateTaskFormPriorityPreview();  }  function resetForm() {    $('editId').value = '';    $('taskName').value = '';    if ($('taskProjectSelect')) $('taskProjectSelect').value = '__new__';    if ($('taskProject')) $('taskProject').value = '';    $('taskChannel').value = '웍스';    $('taskStatus').value = '미진행';    $('taskDesc').value = '';    $('taskDueDate').value = '';    const now = new Date();    fillTimeSelectorsFromDue(now.getHours(), Math.floor(now.getMinutes() / 15) * 15);    setTaskPriorityInputs({ impact: 2, urgency: 2, linkage: 1 });    $('saveBtn').textContent = '업무 저장';  }  function fillForm(task) {    $('editId').value = task.id;    $('taskName').value = task.name;    renderProjectSelectOptions(task.project || '', task.project || '');    $('taskChannel').value = task.channel;    $('taskStatus').value = task.status;    $('taskDesc').value = task.description || '';    $('taskDueDate').value = task.dueDate || '';    const due = parseDueDateTime(task.dueDate, task.dueTime);    if (due) fillTimeSelectorsFromDue(due.getHours(), due.getMinutes());    setTaskPriorityInputs(task);    $('saveBtn').textContent = '수정 저장';    setTab('home');    window.scrollTo({ top: 0, behavior: 'smooth' });  }  function getFormDueTimeValue() {
    ...
    el.innerHTML = list.map((task) => {
      ...
      const dueBadge = getDueBadgeDisplay(task);
      const dueBadgeHtml = dueBadge
        ? '<span class="dday-badge ' + dueBadge.cls + '">' + escapeHtml(dueBadge.label) + '</span>'
        : '<span class="dday-badge dday-empty" aria-hidden="true"></span>';
      return ''
        + '<div class="list-item today-list-item">'
        + '<span class="score-badge today-score-badge ' + priority.cls + '" title="' + escapeHtml(priority.text) + '">' + escapeHtml(priority.label) + '</span>'
        + '<div class="today-task-copy">'
        + '<div class="task-name">' + escapeHtml(task.name) + '</div>'
        + '<div class="task-meta today-meta-line">'
        + '<span class="today-meta-text">' + escapeHtml(task.channel) + ' \u00B7 ' + escapeHtml(formatDueDisplay(task)) + '</span>'
        + dueBadgeHtml
        + '</div>'
        + '</div>'
